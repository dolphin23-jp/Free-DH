...
function simulateBatch(
  ...
  for (let index = 0; index < RUNS; index += 1) {
    const result = simulate(build, enemyId, `t23:${level}:${elite ? 'elite' : 'normal'}:${enemyId}:${index}`)
...
console.log(`T23 diagnostic, ${RUNS} runs per matchup`)
console.log('level\telite\tgroup\tenemy\twin%\ttime\tdamage')
for (const row of rows) {
  console.log(
    `${row.level}\t${row.elite ? 'yes' : 'no'}\t${row.group}\t${row.enemyId} ${row.enemyName}\t${row.winRate}\t${row.averageDuration}\t${row.averageDamage}`,
  )
}

console.log('\nGROUP SUMMARY')
for (const level of LEVELS) {
  for (const elite of level === 0 ? [false] : [false, true]) {
