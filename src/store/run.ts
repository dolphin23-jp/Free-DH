...
function cloneTriggers(triggers: CombatTriggerEffect[] | undefined): CombatTriggerEffect[] | undefined {
...
function cloneInventoryItem(item: RunInventoryItem): RunInventoryItem {
  return {
    ...
    ...(item.resolvedModifiers === undefined
      ? {}
      : { resolvedModifiers: cloneModifiers(item.resolvedModifiers) }),
    ...(item.resolvedTriggers === undefined
      ? {}
      : { resolvedTriggers: cloneTriggers(item.resolvedTriggers) }),
  }
}
...
function cloneBag(bag: RunBagState): RunBagState {
  return {
    columns: bag.columns,
    rows: bag.rows,
...
function cloneStorage(storage: RunStorageState): RunStorageState {
  return {
    capacity: storage.capacity,
    items: storage.items.map(cloneInventoryItem),
  }
}

function cloneResult(result: RunResult | null): RunResult | null {
...
function cloneRunData(data: RunData): RunData {
  return {
    ...
    bag: cloneBag(data.bag),
    storage: cloneStorage(data.storage),
    result: cloneResult(data.result),
  }
}
...
function shuffledEnemyIds(seed: Seed, area: (typeof RUN_AREAS)[number]): string[] {
  const regularIds = enemies
    .filter((enemy) => enemy.area === area && !enemy.isBoss)
    .map((enemy) => enemy.id)
  const bossIds = enemies.filter((enemy) => enemy.area === area && enemy.isBoss).map((enemy) => enemy.id)
  ...
  for (let index = shuffled.length - 1; index > 0; index -= 1) {
    ...
    rngState = step.state
    const targetIndex = Math.floor(step.value * (index + 1))
...
export function createEnemyOrder(seed: Seed): string[] {
  normalizeSeed(seed)
  return RUN_AREAS.flatMap((area) => shuffledEnemyIds(seed, area))
}
...
function validateInventoryItem(item: RunInventoryItem, label: string): void {
  ...
  if (!enemies && item.itemId.trim().length === 0) {
    throw new Error(`${label}.itemId must not be empty`)
  }
  if (item.itemId.trim().length === 0) {
    throw new Error(`${label}.itemId must not be empty`)
  }
  requireNonNegativeFinite(item.runDamageBonus, `${label}.runDamageBonus`)
}
...
function validateInventory(inventory: RunInventorySnapshot): RunInventorySnapshot {
  ...
  return {
    bag: cloneBag(inventory.bag),
    storage: cloneStorage(inventory.storage),
  }
}
...
function dataFromSnapshot(snapshot: RunSnapshot): RunData {
  ...
  requireNonNegativeFinite(snapshot.currentHp, 'currentHp')
  const maxHp = requireNonNegativeFinite(snapshot.maxHp, 'maxHp')
  requireNonNegativeFinite(snapshot.gold, 'gold')
  if (snapshot.currentHp > maxHp) {
  ...
  if (snapshot.phase === 'idle') {
    if (snapshot.runSeed !== null || snapshot.enemyOrder.length !== 0 || snapshot.result !== null) {
    ...
    const expectedOrder = createEnemyOrder(snapshot.runSeed)
    if (!arraysEqual(snapshot.enemyOrder, expectedOrder)) {
...
function requirePhase(state: RunStoreState, expected: RunPhase, action: string): void {
...
function applyRunDamageBonuses(
  ...
  const update = <T extends RunInventoryItem>(item: T): T => {
    const bonus = bonuses[item.instanceId]
    if (bonus === undefined) {
      return item
    }
    requireNonNegativeFinite(bonus, `runDamageBonusByInstanceId.${item.instanceId}`)
    return { ...item, runDamageBonus: bonus }
  }

  return validateInventory({
    bag: {
      ...inventory.bag,
      items: inventory.bag.items.map((item) => update(cloneBagItem(item))),
    },
    storage: {
      ...inventory.storage,
      items: inventory.storage.items.map((item) => update(cloneInventoryItem(item))),
    },
  })
}
...
export function battleResolutionFromCombatState(state: CombatState): BattleResolution {
  ...
  state.player.items
    ...
    .forEach((item) => {
      runDamageBonusByInstanceId[item.instanceId] = item.runDamageBonus
    })
...
export function getCurrentBattleSeed(state: RunData): number | null {
  if (state.runSeed === null || getCurrentEnemyId(state) === null) {
    return null
  }
...
export function createRunStore(snapshot?: RunSnapshot): StoreApi<RunStoreState> {
  ...
  return createStore<RunStoreState>()((set, get) => ({
    ...
    completeBattle: (resolution) => {
      ...
      const maxHp = requireNonNegativeFinite(resolution.playerMaxHp, 'resolution.playerMaxHp')
      const hp = Math.min(
        requireNonNegativeFinite(resolution.playerHp, 'resolution.playerHp'),
        maxHp,
      )
      const gold = requireNonNegativeFinite(resolution.playerGold, 'resolution.playerGold')
      ...
      if (resolution.result === 'playerDefeat') {
        set({
          ...
          bag: inventory.bag,
          storage: inventory.storage,
          result: buildRunResult(state, 'defeated', state.battlesWon, hp, maxHp, gold),
        })
        return
      }
      ...
      if (state.battleIndex === RUN_BATTLE_COUNT - 1) {
        set({
          ...
          bag: inventory.bag,
          storage: inventory.storage,
          result: buildRunResult(state, 'cleared', battlesWon, hp, maxHp, gold),
        })
        return
      }
    ...
    loadSnapshot: (nextSnapshot) => {
      set(dataFromSnapshot(nextSnapshot))
    },
